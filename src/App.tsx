...
function orientedRectBBox(cx:number, cy:number, w:number, h:number, rotDeg:number){
...
export default function App(){
  ...
  const [layers,setLayers]=useState<Layer[]>([
    { id: uid("text"), type:"text", name:"Top text", x:0.5, y:0.07, scale:1, opacity:1, text:"MOTHER OF GOD", fontSize:88, strokePx:12, letterSpacing:0, allCaps:true, rotation:0, visible:true },
  ...
  useEffect(()=>{
    ...
    if(selected && selected.visible!==false){
      ...
      const bbox = (function(cx:number,cy:number,w:number,h:number,rotDeg:number){
        const a=(rotDeg||0)*Math.PI/180, cos=Math.cos(a), sin=Math.sin(a), hw=w/2, hh=h/2;
        const pts=[[-hw,-hh],[hw,-hh],[hw,hh],[-hw,hh]].map(([x,y])=>({x:cx+x*cos-y*sin,y:cy+x*sin+y*cos}));
        const xs=pts.map(p=>p.x), ys=pts.map(p=>p.y);
        const minX=Math.min(...xs), maxX=Math.max(...xs), minY=Math.min(...ys), maxY=Math.max(...ys);
        return {x:minX,y:minY,w:maxX-minX,h:maxY-minY};
      })(width*selected.x,height*selected.y,w,h,rot);
      ctx.strokeRect(bbox.x,bbox.y,bbox.w,bbox.h); ctx.restore();
    }
